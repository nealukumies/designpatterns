    ...
    public void start(Stage stage) {
        ...
        button.setOnAction(e -> {
            if (historyListView == null) {
                ...
                historyListView.setOnMouseClicked(mouseEvent -> {
                    int index = historyListView.getSelectionModel().getSelectedIndex();
                    if (index >= 0 && index < controller.getHistory().size()) {
                        controller.moveToMemento(index);
                    }
                });
                Stage secondaryStage = new Stage();
                Scene secondaryScene = new Scene(historyListView, 200, 300);
                secondaryStage.setScene(secondaryScene);
                secondaryStage.setTitle("Memento History");
                secondaryStage.show();
            }
            updateHistoryList();
        });
    ...
    public void updateHistoryList() {
        if (historyListView != null) {
            ...
            for (int i = 0; i < history.size(); i++) {
                ...
                historyListView.getItems().add("Memento " + (i + 1) + " - " + memento.getCreationDate().toString());
            }
        }
    }
}